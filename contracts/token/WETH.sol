...
contract WETH is ERC20, ERC20Detailed {
    ...
    // Modifiers
    modifier onlyGov() {
        require(msg.sender == governance, "WETH-Token: !governance");
        _;
    }

    constructor () public ERC20Detailed("WETH.finance", "WETH", 18, 100000000 * 10 ** 18) {
    ...
    function mint(address _account, uint256 _amount) public {
        require(minters[msg.sender], "WETH-Token: !minter");
    ...
    function acceptGov()
        external {
        require(msg.sender == pendingGov, "WETH-Token: !pending");
    ...
    function _beforeTokenTransfer(address from, address to, uint256 amount) internal {
        ...
        if (from == address(0)) { // When minting tokens
            require(totalSupply().add(amount) <= cap(), "WETH-Token: Cap exceeded");
        }
    }
}