 ...
 * Beeswap-GXC Pool
 */
contract BeeswapGXCPool is LPTokenWrapper, IRewardDistributionRecipient {
    ...
    function stake(uint256 amount) public checkOpen checkStart updateReward(msg.sender){ 
        require(amount > 0, "Beeswap-GXC-POOL: Cannot stake 0");
        super.stake(amount);
        emit Staked(msg.sender, amount);
    }

    function withdraw(uint256 amount) public checkStart updateReward(msg.sender){
        require(amount > 0, "Beeswap-GXC-POOL: Cannot withdraw 0");
    ...
    modifier checkStart(){
        require(block.timestamp > startTime,"Beeswap-GXC-POOL: Not start");
        _;
    }

    modifier checkOpen() {
        require(open, "Beeswap-GXC-POOL: Pool is closed");
        _;
    }
