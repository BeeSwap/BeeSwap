 ...
 * Beeswap-GOF Pool
 */
contract BeeswapGOFPool is LPTokenWrapper, IRewardDistributionRecipient {
    ...
    function stake(uint256 amount) public checkOpen checkStart updateReward(msg.sender){ 
        require(amount > 0, "Beeswap-GOF-POOL: Cannot stake 0");
        super.stake(amount);
        emit Staked(msg.sender, amount);
    }

    function withdraw(uint256 amount) public checkStart updateReward(msg.sender){
        require(amount > 0, "Beeswap-GOF-POOL: Cannot withdraw 0");
    ...
    modifier checkStart(){
        require(block.timestamp > startTime,"Beeswap-GOF-POOL: Not start");
        _;
    }

    modifier checkOpen() {
        require(open, "Beeswap-GOF-POOL: Pool is closed");
        _;
    }
